...
Проект *[SoftSpiders](https://github.com/softspider)* был создан в начала этого, уже почти прошедшего года.
...
# Зачем проект *SoftSpiders* нужен проекту *Jepria* ? 

## *SoftSpiders* как хостинг прототипов

Как выше было сказано, *SoftSpiders* является базой проектов-прототипов.  
И, как можно узнать из [описания](https://github.com/softspider/softspiders), главным средством систематизации прототипов
служат их свойства, формализованные тегами.  

### Программный продукт как свойство прототипа

Любой программный продукт, в том числе библиотека или фреймворк, такой, как *Jepria* может быть свойством множества
прототипов.
Делая выборки с наборами тегов, включающими тег *jepria*, пользователь будет работать в контексте только *Jepria*.  
Таким образом, если потребуется, механизм тегов *SoftSpiders* может быть использован для организации хостинга прототипов
любого программного продукта, и в первую очередь - хостингом прототипов *Jepria*. 
...
Сейчас *SoftSpiders* имеет около 40 прототипов. Все - на GitHub, хотя это и не является обязательным.
...
До настоящего момента *SoftSpiders* находился в стадии proof-of-concept. Фактом этой публикации он, пожалуй, уже
переходит в режим локального (canary;)) MVP, поскольку выносится на ваш суд - суд не только разработчиков, но и
пользователей, хотя пока только потенциальных.
...
Сегодня у вас, как разработчиков Jepria, есть прекрасная возможность "встать у истоков" *SoftSpiders*.  
